...
const TranspReqListRfq = () => {
    ...
    const rightButtonArr = [{ btn: "Active", func: () => setActiveBtn("Active"), className: "btn btn-success me-1 mb-2" }, { btn: "Archived", func: () => setActiveBtn("Archived"), className: "btn btn-danger mb-2" }]
    ...
    const loadRfqList = () => {
        ...
        GetRfqList(type).then(result => {
            if (result !== undefined) {
                ...
                if (response.success) {
                    const archivedList = response.data.result.filter(item => (item.archieved === true))
                    const rfqList = response.data.result.filter(item => (item.archieved === false))
